...
---
-- @usage
-- sudo kmap -sU -p 1434 --script ms-sql-dac <ip>
--
-- @output
-- | ms-sql-dac:
-- |   SQLSERVER:
...
local function checkPort(host, port)
  local scanport = kmap.get_port_state(host, {number=port, protocol="tcp"})
  if scanport then
...
local function rule_if_browser_open(lib_rule)
  return function (host, ...)
    ...
    local bport = kmap.get_port_state(host, {number=1434, protocol="udp"})
