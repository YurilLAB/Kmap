 ...
 * are specific to Kmap.  Most of this has been moved into nbase so it can *
 * be shared.                                                              *
 *                                                                         *
...
static void init_dll_path()
{
	...
	if (SetDllDirectory == NULL) {
		...
		if (GetModuleFileName(NULL, kmapdir, sizeof(kmapdir)) == 0 ||
		    GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
			pfatal("Error in GetModuleFileName");
		}
		if (SetCurrentDirectory(kmapdir))
...
/* If we find the Npcap driver, allow Kmap to load Npcap DLLs from the "\System32\Npcap" directory. */
...
void win_init()
{
    ...
    if (o.isr00t) {
      if (!start_service("npcap")) {
		...
		{
			error("WARNING: Failed to load wpcap.dll.  Kmap may not function.\n");
			o.have_pcap = false;
		}
#endif
		if(o.debugging)
...
#ifdef _MSC_VER
	__except (1) {
			error("WARNING: Could not import all necessary Npcap functions. You may need to upgrade to the latest version from https://npcap.com. Resorting to connect() mode -- Kmap may not function completely");
		o.have_pcap=false;
		}
#endif
