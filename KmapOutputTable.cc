...
/***************************************************************************
 * KmapOutputTable.cc -- A relatively simple class for organizing Kmap     *
...
KmapOutputTable::KmapOutputTable(int nrows, int ncols) {
  numRows = nrows;
  numColumns = ncols;
  assert(numRows > 0);
  assert(numColumns > 0);
  table = (struct KmapOutputTableCell *) safe_zalloc(sizeof(struct KmapOutputTableCell) * nrows * ncols);
...
KmapOutputTable::~KmapOutputTable() {
  unsigned int col, row;
  struct KmapOutputTableCell *cell;
...
void KmapOutputTable::addItem(unsigned int row, unsigned int column, bool fullrow,
...
void KmapOutputTable::addItem(unsigned int row, unsigned int column, bool copy, const char *item,
                              int itemlen) {
  struct KmapOutputTableCell *cell;
...
void KmapOutputTable::addItemFormatted(unsigned int row,
                                          unsigned int column,
                                          bool fullrow,
                                          const char *fmt, ...) {
  struct KmapOutputTableCell *cell;
  ...
  if (res >= sizeof(buf))
    fatal("KmapOutputTable only supports adding up to 4096 to a cell via %s.", __func__);
...
bool KmapOutputTable::emptyRow(unsigned int nrow) {
        KmapOutputTableCell *cell;
        unsigned int col;
        bool isEmpty = true;
...
char *KmapOutputTable::printableTable(int *size) {
  unsigned int col, row;
