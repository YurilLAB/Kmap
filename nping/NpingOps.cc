 ...
 * determine if has root privileges. */
    ...
    /* TODO: Check this because although kmap does exactly the same, it has a this->have_pcap that may affect to this */
    isr00t=true;
#else
  if (getenv("KMAP_PRIVILEGED") || getenv("NPING_PRIVILEGED"))
    isr00t=true;
  else if (getenv("KMAP_UNPRIVILEGED") || getenv("NPING_UNPRIVILEGED"))
    isr00t=false;
  else
    isr00t = !(geteuid());
#endif
