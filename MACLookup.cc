 ...
 * vendor registered to a MAC address using the kmap-mac-prefixes          *
 * database.                                                               *
 *                                                                         *
...
static void mac_prefix_init() {
  ...
  if (kmap_fetchfile(filename, sizeof(filename), "kmap-mac-prefixes") != 1){
    error("Cannot find kmap-mac-prefixes: Ethernet vendor correlation will not be performed");
    return;
  }
  ...
  o.loaded_data_files["kmap-mac-prefixes"] = filename;
