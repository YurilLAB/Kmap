...
/* The actual mutex returned by the kmap.mutex function.
 ...
 * unlock the mutex.
 * It has 1 upvalue: The kmap.mutex function closure.
 */
 ...
 * exclude directive found in the kmap-service-probes file.
 * */
...
/* unlike set_portinfo() this function sets the port state in kmap.
...
static int finalize_cleanup (lua_State *L, int status, lua_KContext ctx)
{
  ...
  lua_pushliteral(L, "kmap.new_try");
  lua_rawset(L, -3);
...
static int l_fetchfile (lua_State *L)
{
  ...
  if (kmap_fetchfile(buf, sizeof(buf), luaL_checkstring(L, 1)) != 1)
    lua_pushnil(L);
  else
 ...
 * in the queue (waiting to be passed to Kmap).
 *
...
// returns a table with DNS servers known to kmap
...
int luaopen_kmap (lua_State *L)
{
  static const luaL_Reg kmaplib [] = {
    ...
    {"new_dnet", nseU_placeholder}, /* imported from kmap.dnet */
    {"get_interface_info", nseU_placeholder}, /* imported from kmap.dnet */
    {"new_socket", nseU_placeholder}, /* imported from kmap.socket */
    {"mutex", nseU_placeholder}, /* placeholder */
    {"condvar", nseU_placeholder}, /* placeholder */
    {NULL, NULL}
  };

  luaL_newlib(L, kmaplib);
  int kmap_idx = lua_gettop(L);

  nseU_weaktable(L, 0, 0, "v"); /* allow closures to be collected (see l_mutex) */
  lua_pushcclosure(L, l_mutex, 1); /* mutex function */
  lua_setfield(L, kmap_idx, "mutex");

  nseU_weaktable(L, 0, 0, "v"); /* allow closures to be collected (see l_condvar) */
  lua_pushcclosure(L, l_condvar, 1); /* condvar function */
  lua_setfield(L, kmap_idx, "condvar");

  lua_newtable(L);
  lua_setfield(L, kmap_idx, "registry");

  /* Pull out some functions from the kmap.socket and kmap.dnet libraries.
     http://seclists.org/kmap-dev/2012/q1/299. */
  luaL_requiref(L, "kmap.socket", luaopen_nsock, 0);
  /* kmap.socket.new -> kmap.new_socket. */
  lua_getfield(L, -1, "new");
  lua_setfield(L, kmap_idx, "new_socket");
  /* Store kmap.socket; used by nse_main.lua. */
  lua_setfield(L, kmap_idx, "socket");

  luaL_requiref(L, "kmap.dnet", luaopen_dnet, 0);
  /* kmap.dnet.new -> kmap.new_dnet. */
  lua_getfield(L, -1, "new");
  lua_setfield(L, kmap_idx, "new_dnet");
  /* kmap.dnet.get_interface_info -> kmap.get_interface_info. */
  lua_getfield(L, -1, "get_interface_info");
  lua_setfield(L, kmap_idx, "get_interface_info");
  /* Store kmap.socket. */
  lua_setfield(L, kmap_idx, "dnet");

  lua_settop(L, kmap_idx);
