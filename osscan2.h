...
struct seq_info {
  int responses;
  int ts_seqclass; /* TS_SEQ_* defines in kmap.h */
  int ipid_seqclass; /* IPID_SEQ_* defines in kmap.h */
...
class HostOsScanStats {
 ...
 public:
   ...
   * >=1: not self, not same network and kmap has got the icmp reply to the U1 probe.
   *  -1: none of the above situations.
   *
   * Possible values of distance_guess:
   *  -1: kmap fails to get a valid ttl by all kinds of probes.
