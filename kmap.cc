/***************************************************************************
 * kmap.cc -- Currently handles some of Kmap's port scanning features as   *
...
static void printusage() {
  ...
  printf("%s %s ( %s )\n"
         ...
         "  --color=<always|never|auto>: Control terminal color output (default: auto)\n"
         "KMAP FEATURES:\n"
         "  --default-creds: Probe open services for default credentials (auto-enables -sV)\n"
         "  --creds-file <file>: Custom credential wordlist (overrides built-in)\n"
         "  --creds-timeout <sec>: Per-attempt timeout for cred checks (default: 3)\n"
         "  --web-recon: HTTP/S recon on detected web ports (title, headers, paths)\n"
         ...
         "  --tm-no-persist: Do not write the topology graph to <data-dir>/topo.db\n"
         "  --topo-export <file>: Export the persisted topology graph (run --tracemap first)\n"
         "  --topo-format <fmt>: Topo export format: dot or json (default: json)\n"
         "  --topo-around <ip>: Limit export to the neighborhood around an IP\n"
         "  --topo-around-depth <n>: BFS depth for --topo-around (default: 1, range: 1-6)\n"
         "  --topo-asn <number>: Limit export to nodes in a single ASN\n"
         "  --net-cluster <ip>: List IPs sharing TLS/host fingerprints with an IP\n"
         ...
         "  --webxml: Reference stylesheet from Kmap.Org for more portable XML\n"
         "  --no-stylesheet: Prevent associating of XSL stylesheet w/XML output\n"
         "MISC:\n"
         "  -6: Enable IPv6 scanning\n"
         "  -A: Enable OS detection, version detection, script scanning, and traceroute\n"
         "  --datadir <dirname>: Specify custom Kmap data file location\n"
...
static void check_setugid(void) {
  if (getuid() != geteuid())
    error("WARNING: Running Kmap setuid, as you are doing, is a major security risk.\n");
  if (getgid() != getegid())
    error("WARNING: Running Kmap setgid, as you are doing, is a major security risk.\n");
}
#endif
...
void validate_scan_lists(scan_lists &vports, KmapOps &vo) {
...
#endif
  char  *machinefilename, *kiddiefilename, *normalfilename, *xmlfilename, *jsonfilename;
...
void parse_options(int argc, char **argv) {
  ...
  struct option long_options[] = {
    ...
    {"default-creds", no_argument, 0, 0},
    {"creds-file", required_argument, 0, 0},
    {"creds-timeout", required_argument, 0, 0},
    {"web-recon", no_argument, 0, 0},
  ...
  while ((arg = getopt_long_only(argc, argv, "46Ab:D:d::e:Ffg:hIi:M:m:nO::o:P::p:qRrS:s::T:Vv::", long_options, &option_index)) != EOF) {
    ...
    case 0:
      ...
      if (strcmp(long_options[option_index].name, "script") == 0) {
        ...
        if (strcmp(long_options[option_index].name, "max-os-tries") == 0) {
          ...
          o.requested_data_files["kmap-services"] = optarg;
          o.fastscan = true;
        } else if (strcmp(long_options[option_index].name, "versiondb") == 0) {
          o.requested_data_files["kmap-service-probes"] = optarg;
          ...
          /*Kmap Log errors is deprecated and is now always enabled by default.
          ...
          fatal("HTML output is not directly supported, though Kmap includes an XSL for transforming XML output into HTML.  See the man page.");
          ...
          char buf[MAXPATHLEN];
          test_file_name(optarg, long_options[option_index].name);
          Snprintf(buf, sizeof(buf), "%s.kmap", logfilename(optarg, &local_time));
          delayed_options.normalfilename = strdup(buf);
          Snprintf(buf, sizeof(buf), "%s.gkmap", logfilename(optarg, &local_time));
          ...
          fatal("Cannot use --resume with other options. Usage: kmap --resume <filename>");
        } else {
    ...
    case '6':
#if !HAVE_IPV6
      fatal("I am afraid IPv6 is not available because your host doesn't support it or you chose to compile Kmap w/o IPv6 support.");
#else
    ...
    case 'h':
      ...
      break;
    case '?':
      error("See the output of kmap -h for a summary of options.");
      exit(-1);
      break;
    case 'I':
...
void  apply_delayed_options() {
  ...
  if (o.osscan) {
    if (o.af() == AF_INET)
        o.reference_FPs = parse_fingerprint_reference_file("kmap-os-db");
  ...
  if (delayed_options.jsonfilename) {
    o.json_output_file = delayed_options.jsonfilename;
  ...
  if (delayed_options.spoofmac) {
    ...
    /* A zero means set it all randomly.  Anything that is all digits
       ...
       treated as a vendor string for lookup in kmap-mac-prefixes */
    ...
    do {
      ...
      if (!strcasecmp(p, "me")) {
        ...
        if (o.numdecoys + i >= MAX_DECOYS - 1)
          fatal("You are only allowed %d decoys (if you need more redefine MAX_DECOYS in kmap.h)", MAX_DECOYS);
...
int kmap_main(int argc, char *argv[]) {
  ...
  if (!o.resuming) {
    ...
    xml_start_comment();
    xml_write_escaped(" %s %s scan initiated %s as: %s ", KMAP_NAME, KMAP_VERSION, mytime, join_quoted(argv, argc).c_str());
    xml_end_comment();
    xml_newline();

    xml_open_start_tag("kmaprun");
    xml_attribute("scanner", "kmap");
    ...
    xml_attribute("version", "%s", KMAP_VERSION);
    xml_attribute("xmloutputversion", KMAP_XMLOUTPUTVERSION);
    xml_close_start_tag();
    xml_newline();
  ...
  do {
    ...
    for (targetno = 0; targetno < Targets.size(); targetno++) {
      ...
      if (currenths->timedOut(NULL)) {
        ...
        if (o.default_creds) print_default_creds_output(currenths);
        if (o.web_recon)     print_web_recon_output(currenths);
        if (o.cve_map)       print_cve_map_output(currenths);
        printmacinfo(currenths);
...
/* Reads in a (normal or machine format) Kmap log file and gathers enough
   state to allow Kmap to continue where it left off.  The important things
   it must gather are:
...
int gather_logfile_resumption_state(char *fname, int *myargc, char ***myargv) {
  char *filestr;
  s64 filelen;
  char kmap_arg_buffer[4096]; /* roughly aligned with arg_parse limit */
  ...
  /* First goal is to find the kmap args */
  if ((p = strstr(filestr, " as: ")))
    p += 5;
  else
    fatal("Unable to parse supposed log file %s.  Are you sure this is an Kmap output file?", fname);
  ...
  q = strchr(p, '\n');
  if (!q || ((unsigned int) (q - p) >= sizeof(kmap_arg_buffer) - 32))
    fatal("Unable to parse supposed log file %s.  Perhaps the Kmap execution had not finished at least one host?  In that case there is no use \"resuming\"", fname);

  strncpy(kmap_arg_buffer, "kmap --append-output ", sizeof(kmap_arg_buffer));
  if ((q - p) + 21 + 1 >= (int) sizeof(kmap_arg_buffer))
    fatal("0verfl0w");
  memcpy(kmap_arg_buffer + 21, p, q - p);
  kmap_arg_buffer[21 + q - p] = '\0';

  q = strstr(kmap_arg_buffer, "-->");
  if (q) {
    *q = '\0';
     char *unescaped = xml_unescape(kmap_arg_buffer);
     if (sizeof(kmap_arg_buffer) < strlen(unescaped) + 1)
       fatal("0verfl0w");
     memcpy(kmap_arg_buffer, unescaped, strlen(unescaped) + 1);
     free(unescaped);
  }

  if (strstr(kmap_arg_buffer, "--randomize-hosts") != NULL) {
  ...
  *myargc = arg_parse(kmap_arg_buffer, myargv);
  if (*myargc == -1) {
    fatal("Unable to parse supposed log file %s.  Sorry", fname);
  }
    ...
    *q = ' ';
    ...
    if (found) {
      ...
      *q = '"';
      ...
      q = p;
      found = NULL;
      while ((q = strstr(q, "\nKmap scan report for ")))
        found = q = q + 22;

      /*  There may be some later IPs of the form :
          "Kmap scan report for florence (x.x.7.10)" (dns reverse lookup)
          or "Kmap scan report for x.x.7.10".
      */
      if (found) {
        q = strchr(found, '\n');
        if (!q)
  ...
  filestr[filelen - 1] = '\n';
#ifdef WIN32
  win32_munmap(filestr, static_cast<int>(filelen));
#else
...
static int kmap_fetchfile_sub(char *filename_returned, int bufferlen, const char *file);
...
/* Search for a file in the standard data file locations. The result is stored
    ...
    * --datadir
    * $KMAPDIR environment variable
    * User's home Kmap directory:
      - [Windows] %APPDATA%\kmap
      - [Non-Windows] ~/.kmap
    * The directory containing the kmap binary
    * [Non-Windows only]:
      - The directory containing the kmap binary plus "../share/kmap"
      - KMAPDATADIR (usually $prefix/share/kmap)
    */
int kmap_fetchfile(char *filename_returned, int bufferlen, const char *file) {
  ...
  res = kmap_fetchfile_sub(filename_returned, bufferlen, file);

  return res;
}

#ifdef WIN32
static int kmap_fetchfile_userdir(char *buf, size_t buflen, const char *file) {
  char appdata[MAX_PATH];
  int res;

  if (SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, SHGFP_TYPE_CURRENT, appdata) != S_OK)
    return 0;
  res = Snprintf(buf, buflen, "%s\\kmap\\%s", appdata, file);
  if (res <= 0 || res >= buflen)
    return 0;
...
#else
static int kmap_fetchfile_userdir_uid(char *buf, size_t buflen, const char *file, int uid) {
  struct passwd *pw;
  int res;
  ...
  if (pw == NULL)
    return 0;
  res = Snprintf(buf, buflen, "%s/.kmap/%s", pw->pw_dir, file);
...
static int kmap_fetchfile_userdir(char *buf, size_t buflen, const char *file) {
  ...
  res = kmap_fetchfile_userdir_uid(buf, buflen, file, getuid());
  if (res != 0)
    return res;

  if (getuid() != geteuid()) {
    res = kmap_fetchfile_userdir_uid(buf, buflen, file, geteuid());
    if (res != 0)
      return res;
  }
...
static int kmap_fetchfile_sub(char *filename_returned, int bufferlen, const char *file) {
  ...
  if (!foundsomething && (dirptr = getenv("KMAPDIR"))) {
  ...
  if (!foundsomething)
    foundsomething = kmap_fetchfile_userdir(filename_returned, bufferlen, file);
...
#ifndef WIN32
    if (!foundsomething) {
      res = Snprintf(filename_returned, bufferlen, "%s/../share/kmap/%s", dir, file);
      if (res > 0 && res < bufferlen) {
        foundsomething = file_is_readable(filename_returned);
      }
    }
...
#ifndef WIN32
  if (!foundsomething) {
    res = Snprintf(filename_returned, bufferlen, "%s/%s", KMAPDATADIR, file);
    if (res > 0 && res < bufferlen) {
      foundsomething = file_is_readable(filename_returned);
    }
  }
  ...
  if (foundsomething && (*filename_returned != '.')) {
    ...
    if (res > 0 && res < bufferlen) {
      if (file_is_readable(dot_buffer) && !same_file(filename_returned, dot_buffer)) {
        ...
        if (warningcount++ < 1)
#endif
          error("Warning: File %s exists, but Kmap is using %s for security and consistency reasons.  set KMAPDIR=. to give priority to files in your local directory (may affect the other data files too).", dot_buffer, filename_returned);
      }
    }
  }
...
/* Helper for display_kmap_version. Tries to extract a word (presumably a
...
static void display_kmap_version() {
...
#ifndef NOLUA
#ifdef LUA_INCLUDED
  with.push_back(std::string("kmap-liblua-") + get_word_or_quote(LUA_RELEASE, 1));
#else
  ...
  with.push_back(std::string("kmap-libssh2-") + get_word_or_quote(LIBSSH2_VERSION, 0));
#else
...
#if HAVE_LIBZ
#ifdef ZLIB_INCLUDED
  with.push_back(std::string("kmap-libz-") + get_word_or_quote(ZLIB_VERSION, 0));
#else
...
#ifdef PCRE_INCLUDED
  with.push_back(std::string("kmap-libpcre2-") + get_word_or_quote(pcre2_version, 0));
#else
...
#ifdef DNET_INCLUDED
  with.push_back(std::string("kmap-libdnet-") + DNET_VERSION);
#else
