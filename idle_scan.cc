...
/* IPv6 fragment ID sequence algorithms. http://seclists.org/kmap-dev/2013/q3/369.
...
struct idle_proxy_info {
  ...
  int seqclass; /* IP ID sequence class (IPID_SEQ_* defined in kmap.h) */
...
static void initialize_idleproxy(struct idle_proxy_info *proxy, char *proxyName,
  ...
  /* OK, through experimentation I have found that some hosts *cough*
   ...
   changed.  This will also catch the case where the Kmap user is
