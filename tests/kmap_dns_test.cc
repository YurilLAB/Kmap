...
int main()
{
  std::cout << "Testing kmap_dns" << std::endl;
o.debugging = 1;

  int ret = 0;
  std::string target = "scanme.kmap.org";
  ...
  // This is a possible answere for an A query for scanme.kmap.org
  ...
  if(ret) std::cout << "Testing kmap_dns finished with errors" << std::endl;
  else std::cout << "Testing kmap_dns finished without errors" << std::endl;
